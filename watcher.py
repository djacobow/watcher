...
class _ScanQueue():
    ...
    def put(self, l):
        ...
        if l is not None and not (isinstance(l,str) and not len(l)):
            if self.disper:
                self.disper.put(v)
...
class Watcher():
    ...
    def wait_subp_done(self, timeout=0):
        ...
        while time.time() < (now + timeout) or timeout==0:
            if self.retcode is not None:
                return self.retcode
        raise WatcherTimeoutException(f'ran out of time wating for {self.name} to complete')
    ...
    def subprocess(self, cmdargs, *args, **kwargs):
        ...
        def our_sendall(s,b):
            s.write(b.decode('utf-8'))
            s.flush()
 
        self.istream.sendall = types.MethodType(our_sendall, self.istream)
    ...
    def ssh(self, user, host, *args, **kwargs):
        ...
        ssh_args = ['ssh', '-t', '-t', '-o', 'StrictHostKeyChecking=no', '{}@{}'.format(user,host)]
        if port is not None:
           ssh_args += ['-p', str(port)]

        return self.subprocess(
            ssh_args, *args, **kwargs
        )
    ...
    def send(self, *args, **kwargs):
        ...
        if self.istream is not None:
            ...
            if raw is not None:
                ...
                omsg = ' '.join([
                    s.strip() for s in [
                        x if isinstance(x,str) else str(x) for x in args
                    ] 
                ])
                omsg += '\r\n'
